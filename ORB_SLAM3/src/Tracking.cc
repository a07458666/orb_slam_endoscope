...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        if(mpCamera->ReconstructWithTwoViews(mInitialFrame.mvKeysUn,mCurrentFrame.mvKeysUn,mvIniMatches,Tcw,mvIniP3D,vbTriangulated))
        {
            ...
            // AndySu Set New map init pose
            long unsigned int init_id = mpAtlas->GetCurrentMap()->GetInitKFid();
            ...
            for(Map* pMap : mpAtlas->GetAllMaps())
            {
                if(pMap->GetAllKeyFrames().size() > 0)
                {
                    ...
                    {
                        mInitialFrame.SetPose(keyFrame->GetPose());
                        mCurrentFrame.SetPose(Tcw * keyFrame->GetPose());
                    }
                }
            }
            // cout << "mInitialFrame : " << mInitialFrame.GetPose().matrix() << endl;
            // cout << "mCurrentFrame : " << mCurrentFrame.GetPose().matrix() << endl;
